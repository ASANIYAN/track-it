    ...
    <section>
      ...
      <section className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-3 justify-center md:justify-start items-center gap-5 mt-2.5">
        <section className="w-[200px]">
          <CustomSelect method={method} name="notification" options={options} />
        </section>
        {!isCustom && (
          <>
            <section className="w-[200px]">
              <CustomTimeInput name="from" label="From" method={method} />
            </section>
            <section className=" border-black w-[200px]">
              <CustomTimeInput name="to" label="To" method={method} />
            </section>
          </>
        )}
      </section>
      {!isCustom && (
        <p className="text-xs font-normal text-color7 dark:text-white mt-4 text-center sm:text-start">
          {" "}
      ...
      {isCustom && (
        <section className="mt-5">
          {days.map((day) => (
            <section
              key={day}
              className="mt-4 mx-auto flex flex-wrap items-center justify-center md:justify-start gap-y-2.5 gap-x-20"
            >
              ...
              <section className="flex flex-wrap items-center gap-y-2.5 gap-x-4">
                <section className="xl:w-[200px]">
                  <CustomTimeInput name="from" method={method} />
                </section>
                <section className="xl:w-[200px]">
                  <CustomTimeInput name="to" method={method} />
                </section>
              </section>
            </section>
          ))}
      ...
      <section className="mt-8">
        ...
        <section className="mt-5">
          <CustomRadio
            ...
            method={method}
          />
          <CustomRadio name="mentions" label="Mentions only" method={method} />
          <CustomRadio
            name="assigned"
            label="Assigned a new task"
            method={method}
          />
          <CustomRadio name="tasks" label="Tasks due today" method={method} />
        </section>
      </section>
      ...
      <section className="mt-8">
        ...
        <section className="mt-5">
          <CustomRadio name="task" label="Task Update" method={method} />
          <CustomRadio name="mentions" label="Mentions only" method={method} />
          <CustomRadio
            name="assigned"
            label="Assigned a new task"
